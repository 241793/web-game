...
export class ObstacleSystem {
    ...
    _createBuilding(x, z, width, depth, height, terrain, color = 0x8a8a7a) {
        ...
        // === 二楼 + 楼梯（对足够高、足够大的建筑） ===
        // 二楼地板高度：留一楼 ≥2.5m 净高 + 二楼 ≥2.2m 净高
        if (height >= 6 && width >= 9 && depth >= 8) {
            const floor2Y = 3.0;                                // 二楼地板相对建筑 base 的高度
            const openingW = 2.6;                                // 楼梯井宽度（与楼梯同宽）
            const stairW = 2.6;                                  // 楼梯宽度
            // 楼梯靠 +X 或 -X 侧（确定性 hash，重建稳定）
            const stairSide = (Math.floor(Math.abs(x * 0.7 + z * 0.3)) % 2 === 0) ? 1 : -1;
            const innerDepth = depth - wallThickness * 1.6;
            const innerWidth = width - wallThickness * 1.6;

            // 二楼地板：沿 X 方向分两段，靠楼梯侧留出 openingW 宽的楼梯井
            const slabXSpan = innerWidth - openingW;
            const slabCenterXOff = -stairSide * openingW / 2;
            addPart(
                'floor2_slab',
                x + slabCenterXOff,
                y + floor2Y,
            ...
            // 二楼地板边缘（楼梯井两侧）封板，视觉闭合 + 防穿模
            addPart('floor2_rim_a', x + stairSide * (width / 2 - openingW / 2 - wallThickness / 2), y + floor2Y, z, 0.05, 0.18, innerDepth, trimMat, false);

            // 楼梯：沿 -Z 侧墙拾级而上，从一楼地面升到二楼地板
            // 台阶高度 0.25m（远低于 0.7m step-up 限制），深度 0.42m（脚能踩稳）
            const targetStepH = 0.25;
            const stepD = 0.42;
            const stepCount = Math.max(8, Math.ceil(floor2Y / targetStepH));
            const actualStepH = floor2Y / stepCount;
            const stairsStartZ = z - depth / 2 + wallThickness + 0.6;  // 从 -Z 里侧开始
            const stairsX = x + stairSide * (width / 2 - wallThickness - stairW / 2 - 0.05);
            for (let i = 0; i < stepCount; i++) {
                const stepY = y + actualStepH * (i + 1);
                const stepZPos = stairsStartZ + i * stepD;
                addPart(
                    `stairs_${i}`,
                    stairsX,
                    stepY - actualStepH / 2,
            ...
            // 楼梯顶端到二楼地板的过渡平台（覆盖最后一级到 floor2_slab 之间）
            const topStepZ = stairsStartZ + (stepCount - 1) * stepD;
            const platformD = (z + innerDepth / 2 - wallThickness) - (topStepZ + stepD / 2);
            if (platformD > 0.3) {
                addPart(
                    'stairs_top_platform',
                    stairsX,
                    y + floor2Y - 0.09,
                    topStepZ + stepD / 2 + platformD / 2,
                    stairW,
                    0.18,
                    platformD,
                    floorMat,
            ...
            // 二楼楼梯井护栏（沿开口侧，从二楼地板升到天花板）
            const railH = 1.0;
            addPart(
                'floor2_railing',
                x + stairSide * (width / 2 - openingW - wallThickness - 0.06),
                y + floor2Y + railH / 2,
                z,
                0.08,
                railH,
            ...
            // 楼梯井另一侧护栏（沿楼梯长度，跟随台阶上升）
            // 用阶梯式：每级台阶上方加一根立柱 + 横杆，组成楼梯侧护栏
            const railOuterX = stairsX - stairSide * (stairW / 2 + 0.08);
            for (let i = 0; i < stepCount; i++) {
                const stepTopY = y + actualStepH * (i + 1);
                const stepZPos = stairsStartZ + i * stepD;
                // 立柱：从台阶顶向上 1.0m
                addPart(
                    `stairs_rail_post_${i}`,
            ...
            for (let i = 0; i < stepCount - 1; i++) {
                const yA = y + actualStepH * (i + 1) + 0.9;
                const yB = y + actualStepH * (i + 2) + 0.9;
                const zA = stairsStartZ + i * stepD;
                const zB = stairsStartZ + (i + 1) * stepD;
                const yMid = (yA + yB) / 2;
                const zMid = (zA + zB) / 2;
                const dy = yB - yA;
                const dz = zB - zA;
            ...
            // === 二楼家具 ===
            const f2y = y + floor2Y + 0.09;  // 二楼地板顶面
            const f2LeftX = x - stairSide * (width / 4 + 0.3);  // 远离楼梯井的一侧
            // 二楼床（靠墙）
            addPart('f2_bed', f2LeftX, f2y + 0.35, z + depth * 0.28, 1.4, 0.7, 2.0, trimMat, true);
            addPart('f2_bed_pillow', f2LeftX, f2y + 0.75, z + depth * 0.28 - 0.7, 0.9, 0.1, 0.5, trimMat, false);
            // 二楼桌子
            addPart('f2_desk', x + stairSide * (width * 0.05), f2y + 0.38, z - depth * 0.25, 1.8, 0.76, 0.9, trimMat, true);
            addPart('f2_desk_top', x + stairSide * (width * 0.05), f2y + 0.76, z - depth * 0.25, 1.85, 0.06, 0.95, trimMat, false);
            // 椅子
            addPart('f2_chair_seat', x + stairSide * (width * 0.05), f2y + 0.48, z - depth * 0.25 + 0.7, 0.5, 0.06, 0.5, trimMat, true);
            addPart('f2_chair_back', x + stairSide * (width * 0.05), f2y + 0.75, z - depth * 0.25 + 0.9, 0.5, 0.5, 0.06, trimMat, true);
            // 二楼书架
            addPart('f2_shelf', f2LeftX + stairSide * 0.3, f2y + 0.75, z - depth * 0.35, 1.0, 1.5, 0.4, trimMat, true);
            // 二楼沙发
            addPart('f2_sofa', x + stairSide * (width * 0.18), f2y + 0.35, z + depth * 0.15, 1.8, 0.7, 0.7, trimMat, true);
            // 二楼箱子堆
            addPart('f2_crate_stack', x - stairSide * (width * 0.3), f2y + 0.45, z + depth * 0.05, 0.9, 0.9, 0.9, trimMat, true);
            addPart('f2_crate_top', x - stairSide * (width * 0.3), f2y + 1.35, z + depth * 0.05, 0.7, 0.7, 0.7, trimMat, true);

            // 二楼窗户（前后墙的高位开窗，让二楼有采光）
            // 复用 addWindowedWallX 不行（它会重新分割整墙），这里直接在高处加视觉窗框
            const sillH2 = floor2Y + 0.6;
            const openingH2 = 1.1;
            const windowW2 = Math.min(Math.max(1.4, innerWidth * 0.4), 2.4);
            for (const wz of [z - depth / 2 + wallThickness / 2, z + depth / 2 - wallThickness / 2]) {
                // 不重复加碰撞墙——只是装饰窗框 + 玻璃
                addPart('f2_window_sill', x, y + sillH2 / 2, wz, windowW2 + 0.2, sillH2 - floor2Y - 0.1, 0.06, trimMat, false);
                addPart('f2_window_lintel', x, y + sillH2 + openingH2 + 0.06, wz, windowW2 + 0.2, 0.12, 0.06, trimMat, false);
                addPart('f2_window_left', x - windowW2 / 2 - 0.08, y + floor2Y + (sillH2 - floor2Y + openingH2) / 2 + 0.55, wz, 0.1, openingH2, 0.06, trimMat, false);
                addPart('f2_window_right', x + windowW2 / 2 + 0.08, y + floor2Y + (sillH2 - floor2Y + openingH2) / 2 + 0.55, wz, 0.1, openingH2, 0.06, trimMat, false);
                const pane = addPart('f2_window_glass', x, y + sillH2 + openingH2 / 2, wz, windowW2 * 0.9, openingH2 * 0.9, 0.03, glassMat, false);
                markGlass(pane);
            }

            // 二楼靠楼梯侧的内墙（隔出楼梯间，留 1.2m 宽门洞）
            const doorGap = 1.2;
            const innerWallSpan = (innerDepth - doorGap) / 2;
            for (const sgn of [-1, 1]) {
                addPart(
                    `floor2_partition_${sgn > 0 ? 'f' : 'b'}`,
                    x + stairSide * (width / 2 - openingW - wallThickness - 0.06),
                    y + floor2Y + (height - floor2Y) / 2,
                    z + sgn * (doorGap / 2 + innerWallSpan / 2),
                    0.1,
                    height - floor2Y - 0.2,
                    innerWallSpan,
                    wallMat,
